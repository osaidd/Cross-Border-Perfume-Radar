...
def encode_features(df: pd.DataFrame) -> pd.DataFrame:
    X = pd.get_dummies(df[["brand", "line", "concentration"]].map(lambda s: str(s).strip().lower()))
...
def train_models(pairs: pd.DataFrame):
    ...
    norm = pairs.assign(
        brand=pairs["brand"].map(_normalize_str), line=pairs["line"].map(_normalize_str)
    )
    ...
    ratio_pred = (
        pairs.apply(
            lambda r: _pick_ratio(
                _normalize_str(r["brand"]),
                _normalize_str(r["line"]),
                ratio_by_brand_line,
                ratio_by_brand,
        ...
        * pairs["retail_aed"].astype(float).values
    )
...
def predict_for_retail(
    retail_df: pd.DataFrame, models: dict, known_wholesale_keys: set
) -> pd.DataFrame:
    ret = retail_df.copy()
    ret["_key"] = _key(ret)
    ...
    def pick_ratio(row):
        ...
        return _pick_ratio(
            brand,
            line,
            models["ratio_by_brand_line"],
            models["ratio_by_brand"],
            models["ratio_global"],
        )
    ...
    ret["predicted_wholesale_aed"] = (
        0.4 * ret["ratio_pred"] + 0.3 * ret["ridge_pred"] + 0.3 * ret["rf_pred"]
    )
...
def evaluate_model(pairs: pd.DataFrame) -> None:
    ...
    merged["actual_flag"] = merged["wholesale_aed"] / merged["retail_aed"] <= SPREAD_THRESHOLD
...
def main():
    ...
    if len(pairs) > 0:
        ...
        models = {
            ...
            "rf": RandomForestRegressor(n_estimators=10, random_state=42).fit(
